...
if (isset($_SESSION['id_usuario']) && !empty($_SESSION['id_usuario'])) {
    ...
    if (mysqli_num_rows($result_usuario) == 0) {
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            if (isset($_POST["tipo"]) && isset($_POST["link"]) && isset($_POST["observacao"]) && !empty($_POST["tipo"]) && !empty($_POST["link"]) && isset($_POST["idsocial"]) && !empty($_POST["idsocial"])) {

                // Obtém os valores dos campos do formulário
                $tipo = $_POST["tipo"];
                $link = $_POST["link"];
                $id_edit = $_POST["idsocial"];
                ...
                $sql = "UPDATE `social` SET Tipo = '$tipo', Link = '$link', Observacao = '$observacao', IdUsuario = '$id_usuario_sessao' WHERE IdSocial = '$id_edit'";
